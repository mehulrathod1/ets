...
class EmployeePaidInvoice extends StatefulWidget {
  const EmployeePaidInvoice({Key? key}) : super(key: key);

  @override
  State<EmployeePaidInvoice> createState() => _EmployeePaidInvoiceState();
}

class _EmployeePaidInvoiceState extends State<EmployeePaidInvoice> {
  ...
  Future initialize(BuildContext context, String search) async {
    ...
    await employeeInvoiceController
        .getEmployeePaidInvoice(context, search: search, page: 1)
        .then((value) {
      setState(() {
        if (value != null) {
          invoiceModel = value;
