...
class CompanyUnPaidInvoice extends StatefulWidget {
  const CompanyUnPaidInvoice({Key? key}) : super(key: key);

  @override
  State<CompanyUnPaidInvoice> createState() => _CompanyUnPaidInvoiceState();
}

class _CompanyUnPaidInvoiceState extends State<CompanyUnPaidInvoice> {
  ...
  Future initialize(BuildContext context, String search) async {
    ...
    await invoiceController
        .getCompanyUnpaidInvoice(context, search: search, page: 1)
        .then((value) {
      setState(() {
        if (value != null) {
